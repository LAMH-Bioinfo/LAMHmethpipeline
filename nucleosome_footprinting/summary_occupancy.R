...
df <- fread(paste0(list_sample[i], '.out.occupancy.tsv'), header=TRUE)
df <- data.frame(df)
df <- df[,c(1,5)]
...
write.table(df_out, paste0(batch_id, '_occupancy.tsv'), col.names=TRUE, row.names=FALSE, quote=FALSE, sep='\t')

...
def disp(txt, nt=0):
    if not args.quiet: print('[methratio] @%s \t%s' %(time.asctime(), txt),file=sys.stderr)

def mbs(array_meth):
    n_cpg = len(array_meth)
    i = 0
    n_successive = 0
    successive_cpg = []
    while i < n_cpg:
        if array_meth[i] == 1: n_successive += 1
        if array_meth[i] == 0 or (i == n_cpg - 1):
            successive_cpg.append(n_successive)
            n_successive = 0
        i += 1
    sum_succ_cpg = 0
    for j in range(len(successive_cpg)):
        sum_succ_cpg += successive_cpg[j]*successive_cpg[j]
    mbs_value = float(sum_succ_cpg)/float(n_cpg*n_cpg)
    return(mbs_value)
...
if args.mode == 'trad':
    ...
    for cr in ref:
        ...
        meth_read[cr] = array.array('f', [0]) * len(ref[cr]) #CHALM
...
if args.mode == "CHALM":
    ...
    for infile, sam_format, fin in pipes:
        ...
        for line in fin:
            ...
            if len(map_info) == 0: continue
            seq, strand, cr, pos = map_info
            depth_read_cr = depth_read[cr]    #JX
            pos2 = pos + len(seq)
            nmap += 1
            meth_read_cr = meth_read[cr]    #JX
            refseq, refmarkcr = ref[cr], refmark[cr]
            match, convert, rc_match, rc_convert = BS_conversion[strand]
            index, readmeth, cytosine_number = refseq.find(match, pos, pos2), 0, 0
            #
            array_cpg_meth = []
            while index >=0:
                if seq[index-pos] == match and refmarkcr[index] in seq_context: array_cpg_meth.append(1)
                if seq[index-pos] == convert and refmarkcr[index] in seq_context: array_cpg_meth.append(0)
                index = refseq.find(match, index+1, pos2)
            if len(array_cpg_meth) < 2 : continue
            meth_read_cr[pos] += mbs(array_cpg_meth)
            depth_read_cr[pos] += 1

            # while index >=0 and readmeth < args.readmeth:
            #     if seq[index-pos] == match and refmarkcr[index] in seq_context: readmeth += 1
            #     if seq[index-pos] == convert and refmarkcr[index] in seq_context: cytosine_number += 1
            #     index = refseq.find(match, index+1, pos2)
            # if readmeth >= args.readmeth: 
            #     meth_read_cr[pos] += 1
            #     depth_read_cr[pos] += 1
            # elif cytosine_number > 0:
            #     depth_read_cr[pos] += 1  
    ...
    disp('writing %s ...' % args.outfile)
    if not args.no_header: 
        fout.write('chr\tstart\tend\tratio\ttotal_read_count\tmethylated_read_count\tCI_lower\tCI_upper\n')
    z95, z95sq = 1.96, 1.96 * 1.96
    for line in open(args.region):
        cr, start, end = line.split('\t')[0], int((int(line.split('\t')[1])-args.read_length*0.5)), int((int(line.split('\t')[2])-args.read_length*0.5))
        total_count, meth_count, depth_read_cr, meth_read_cr = 0, 0, depth_read[cr], meth_read[cr]
        for i in range(start, end+1):
            total_count += depth_read_cr[i]
            meth_count += meth_read_cr[i]
        try: ratio = float(meth_count)/float(total_count)
        except ZeroDivisionError: continue
        pmid = ratio + z95sq / (2 * total_count)
        sd = z95 * ((ratio*(1-ratio)/total_count + z95sq/(4*total_count*total_count)) ** 0.5)
        denorminator = 1 + z95sq / total_count
        CIl, CIu = (pmid - sd) / denorminator, (pmid + sd) / denorminator
        fout.write('%s\t%d\t%d\t%.3f\t%d\t%.3f\t%.3f\t%.3f\n' % (cr, int(line.split('\t')[1]), int(line.split('\t')[2]), ratio, total_count, meth_count, CIl, CIu))                                
    if args.outfile != 'STDOUT': fout.close()
